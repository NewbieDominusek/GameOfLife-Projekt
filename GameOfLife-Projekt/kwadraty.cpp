...
void TileMap::updateLinie() {
	...
	int yOffset = (cellMap->width + 1) * 2; //od którego indeksu w tablicy s¹ dane linii poziomych
	...
	for (int y = 0; y < (cellMap->height + 1) * 2; y += 2) {		//Poziome linie, przechodzi po parach vertexów w tablicy, czêœæ tablicy dla linii poziomych jest po czêœci dla linii pionowych, dlatego jest + yOffset
		linie[y + yOffset].position = sf::Vector2f(0, y / 2 * rozmiarY + marginTop);		//trzeba jeszcze uwzglêdniæ margines od góry
		linie[y + 1 + yOffset].position = sf::Vector2f(cellMap->width * rozmiarX, y / 2 * rozmiarY + marginTop);//trzeba jeszcze uwzglêdniæ margines od góry
...
void TileMap::HandleEvent(sf::Event event) {	//handlowanie eventów
	...
	case sf::Event::MouseButtonPressed:	//naciœniêto myszkê
		if (event.mouseButton.y > marginTop / origH * oknoH && event.mouseButton.y < oknoH - marginBottom / origH * oknoH) {	//sprawdzamy czy naciœniêto na planszê (sprawdzamy tylko y, bo x jest na ca³¹ szerokoœæ i skalujemy do rozmiaru okna)
			...
			if (x > cellMap->width || y > cellMap->height) break;	//sprawdzamy czy wartoœci x i y s¹ w zakresie rozmiaru mapy, tak na wszelki wypadek
			...
			numer = (int)y * cellMap->width + (int)x; //obliczamy który index zmieniæ
			if (event.mouseButton.button == sf::Mouse::Left) {
				if (!pasteInEnabled) cellMap->map[numer] = (cellMap->map[numer] == 1) ? 0 : 1; //jeœli nie jest w³¹czone wklejanie, to po prostu prze³¹czamy stan komórki
				else {
	...
	case sf::Event::MouseMoved: //jeœli tylko ruszono myszk¹, robimy pocz¹tkowo to samo co przy naciœniêciu PPM
		...
		if (event.mouseMove.y > marginTop / origH * oknoH && event.mouseMove.y < oknoH - marginBottom / origH * oknoH) {	//sprawdzamy czy najechano na planszê (sprawdzamy tylko y, bo x jest na ca³¹ szerokoœæ i skalujemy do rozmiaru okna)
			...
			if (x > cellMap->width || y > cellMap->height) break;	//sprawdzamy czy wartoœci x i y s¹ w zakresie rozmiaru mapy
...
void TileMap::hologram(bool clicked) {
	for (int i = 0; i < cellMap->height * cellMap->width; i++) cellMap->map[i] = cellMap->map[i] % 2; //ustaw planszê do stanu z danymi 0 lub 1

	if (!showHolo) return; //sprawdŸ czy mamy w ogóle wyœwietliæ hologram
	if (holoWidth > cellMap->width || holoHeight > cellMap->height) return; //czy hologram mieœci siê w planszy
	
	//jeœli wszystko siê zgadza

	for (int row = 0; row < holoHeight; row++) { //przechodzimy po hologramie
		for (int col = 0; col < holoWidth; col++) {
			if (!cellMap->zawijanie && ((row + holoY >= cellMap->height) || (col + holoX >= cellMap->width))) continue; //jeœli wyst¹pi³o zawijanie a mamy je wy³¹czone, to pomiñ komórkê
			int num = ((row + holoY >= cellMap->height) ? row + holoY - cellMap->height : row + holoY) * cellMap->width + ((col + holoX >= cellMap->width) ? col + holoX - cellMap->width : col + holoX); //liczymy numer komórki (pozycja myszki + miejsce w hologramie) i uwzglêdniamy zawijanie
			if (clicked) cellMap->map[num] = holoData[row * holoWidth + col]; //jeœli klikniêto, to zamiast hologramu wklej wzór do tablicy g³ównej

			//inaczej rysuj hologram:
			else if (cellMap->map[num] == 0) cellMap->map[num] = (holoData[row * holoWidth + col] == 0) ? 8 : 6;	//jeœli komórka jest martwa
			else cellMap->map[num] = (holoData[row * holoWidth + col] == 0) ? 7 : 9;		//jeœli jest ¿ywa
		}
	}
}